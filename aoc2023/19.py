...
def calc_combinations(bounds, wf_name, depth):
	if wf_name == 'R':
		return 0
	if wf_name == 'A':
		result = 1
	...
	for rule in conditions[wf_name]:
		if len(rule) == 1:
			combinations += calc_combinations(copy.deepcopy(updated_bounds), rule[0], depth+1)
			break
		...
		if ((condition == '<' and updated_bounds[category][1] < value) or
			...
			combinations += calc_combinations(copy.deepcopy(updated_bounds), next_wf, depth+1)
			break
		if ((condition == '<' and updated_bounds[category][0] >= value) or
			...
			continue
		if condition == '<':
			# updated_bounds[category][0] < value <= updated_bounds[category][1]
			new_bounds = copy.deepcopy(updated_bounds)
			new_bounds[category][1] = value-1
			combinations += calc_combinations(new_bounds, next_wf, depth+1)
...
print(calc_combinations(bounds, 'in', 0))
